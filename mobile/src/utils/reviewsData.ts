...
export function generateSeedReviews(placeId: string, placeName: string = 'Monument', baseRating: number = 4.6): ReviewItem[] {
...
export function computeReviewStats(reviews: ReviewItem[], fallbackRating: number = 4.6): ReviewStats {
  ...
  const averageRating = totalReviews > 0 
    ? Number((sum / totalReviews).toFixed(1)) 
    : fallbackRating;

  const percentages: Record<number, number> = {
    5: totalReviews > 0 ? Math.round((distribution[5] / totalReviews) * 100) : 75,
    4: totalReviews > 0 ? Math.round((distribution[4] / totalReviews) * 100) : 20,
    3: totalReviews > 0 ? Math.round((distribution[3] / totalReviews) * 100) : 5,
...
export async function getLocalReviews(placeId: string, placeName: string = 'Monument', baseRating: number = 4.6): Promise<{ reviews: ReviewItem[]; stats: ReviewStats }> {
  try {
    const customKey = `user_reviews_${placeId}`;
    const stored = await safeStorage.getItem(customKey);
    const userCustomReviews: ReviewItem[] = stored ? JSON.parse(stored) : [];

    const baseSeed = generateSeedReviews(placeId, placeName, baseRating);
    const combined = [...userCustomReviews, ...baseSeed];
    const stats = computeReviewStats(combined, baseRating);

    return { reviews: combined, stats };
  } catch (e) {
    const fallback = generateSeedReviews(placeId, placeName, baseRating);
    return { reviews: fallback, stats: computeReviewStats(fallback, baseRating) };
  }
}

export async function saveLocalReview(
  placeId: string, 
  review: { userName?: string; rating: number; title?: string; comment: string; visitType?: string; badge?: string }
): Promise<ReviewItem> {
  const customKey = `user_reviews_${placeId}`;
  const stored = await safeStorage.getItem(customKey);
  const current: ReviewItem[] = stored ? JSON.parse(stored) : [];

  const newReview: ReviewItem = {
    id: `local-rev-${Date.now()}-${Math.random().toString(36).slice(2, 6)}`,
    placeId,
    userName: review.userName?.trim() || 'Verified Tourist',
    rating: Math.min(5, Math.max(1, review.rating)),
    title: review.title?.trim() || 'Wonderful Monument Visit',
    comment: review.comment.trim(),
    badge: review.badge || 'Verified Visitor',
    visitType: review.visitType || 'Family',
    helpfulCount: 0,
    createdAt: new Date().toISOString(),
  };

  const updated = [newReview, ...current];
  await safeStorage.setItem(customKey, JSON.stringify(updated));
  return newReview;
}
